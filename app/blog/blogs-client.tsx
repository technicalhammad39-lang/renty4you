...
import { Blog, subscribeToBlogs } from "@/lib/firebase/firestore";
...
export default function BlogsClient({ initialBlogs }: { initialBlogs: Blog[] }) {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToBlogs((liveData) => {
      setBlogs(liveData);
    });
    return () => unsubscribe();
  }, []);
      ...
      {blogs.map((blog) => (
        <Link href={`/blog/${blog.slug}`} key={blog.id} className="group flex flex-col bg-white dark:bg-slate-900 rounded-[32px] overflow-hidden border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-all duration-500 ease-out hover:-translate-y-2">
          ...
          <div className="p-8 flex flex-col flex-grow">
            ...
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-4 line-clamp-2 leading-snug group-hover:text-primary transition-colors">
              {blog.title}
            </h3>
            ...
            <div className="pt-6 border-t border-slate-100 dark:border-slate-800 mt-auto flex justify-between items-center">
              ...
              <span className="w-10 h-10 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-colors">
