...
export function SearchBar({ variant = "full" }: { variant?: "full" | "compact" }) {
  ...
  return (
    ...
    {/* MOBILE FILTER MODAL - Moved outside to prevent 'transform' containing block issues from framer-motion */}
    {/* MOBILE FILTER MODAL - Rendered in Portal to prevent 'transform' clipping issues */}
        ...
        {mobileModalOpen && (
          ...
          >
            <motion.div 
              ...
              className="relative w-full bg-white dark:bg-[#0B1220] rounded-t-[32px] flex flex-col max-h-[90vh] overflow-hidden shadow-[0_-20px_50px_rgba(0,0,0,0.5)]"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              ...
              {/* Sticky Footer */}
              <div className="p-4 bg-white dark:bg-[#0B1220] border-t border-border-subtle absolute bottom-0 left-0 right-0 z-10 shrink-0">
                <button 
                  onClick={() => {
